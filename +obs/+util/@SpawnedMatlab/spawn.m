function spawn(S,host,messengerlocalport,messengerremoteport,...
    ...
    if S.Logging
        ...
        if strcmpi(S.RemoteTerminal,'none')
            ...
            %  terminates. This may not be an issue because
            %  the matlab process inside the terminal, or with an X channel
            %  is terminated too, anyway.
    ...
    messengerperiodicpush=...
       ['MasterMessenger.PeriodicQueries(1).Properties={''setAgain(''''ExecutingCommand'''')''};', ...
        'MasterMessenger.PeriodicQueries(1).Period=10;', ...
    ...
    if strcmpi(S.RemoteMessengerFlavor,'listener') ||...
        ...
        messengercommand = ...
            ...
            '%s MasterMessenger.start;'],S.LocalHost,...
            S.MessengerLocalPort,S.MessengerRemotePort,S.Id,messengerperiodicpush);
    else
        messengercommand = ...
            sprintf(['MasterMessenger=obs.util.Messenger(''%s'',[%d],%d,''%s'');'...
            'MasterMessenger.PushPropertyChanges=true;'...
            '%s MasterMessenger.connect;'],S.LocalHost,...
            S.MessengerLocalPort,S.MessengerRemotePort,S.Id,messengerperiodicpush);
    end
