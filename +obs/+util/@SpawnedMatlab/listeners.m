function [Mpids,Rpids]=listeners(SV)
...
%  multiple listeners, etc.
    ...
    for i=1:numel(SV)
        ...
        [s,r]=system(sprintf('ssh %s %s%s " "', S.SshOptions, user, S.Host));
        ...
        if (s==0 || s==1)
            ...
            a=split(a,newline);
            for j=1:numel(a)
                Mpids=[Mpids;sscanf(a{j},'%d')];
            end
        else 
            S.reportError('cannot execute commands on %s',S.Host)
        end
        
        if nargout>1 && (s==0 || s==1) % return status can be 1 if lsof didn't find the port in use
